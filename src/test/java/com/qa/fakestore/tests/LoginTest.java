...
public class LoginTest extends BaseTest{
	...
	public void userLoginTest() {
		...
		Response responsePost = restClient.post(FAKESTORE_API_USER_ENDPOINT, user, "javascript", true);
		responsePost.then().assertThat().statusCode(APIHttpStatus.OK_200.getCode());
		int userID = responsePost.jsonPath().get("id");
		System.out.println(userID);
		
		//get the user with the user ID -- GET
		restClient = new RestClient(prop, prop.getProperty("baseURI"));
		Response responseGet = restClient.get(FAKESTORE_API_USER_ENDPOINT + "/" + userID, true);
		...
		Response responseAuth = restClient.post(FAKESTORE_API_LOGIN_ENDPOINT, requestBody, "json", true);
